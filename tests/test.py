...
class TestApplyKernels(unittest.TestCase):
    ...
    def test_apply_filter_on_color_img(self): # Test method 2
        ...
        self.assertRaises(IndexError, imkernel.apply_filter_on_color_img, X_train[0], 'blur', True)
        self.assertRaises(IndexError, imkernel.apply_filter_on_color_img, np.array([1, 2, 3]), 'blur', True)
        self.assertRaises(ValueError, imkernel.apply_filter_on_color_img, X_train, 'blur', True)
        self.assertRaises(KeyError, imkernel.apply_filter_on_color_img, flower_img, 'blure', True)
        self.assertRaises(AttributeError, imkernel.apply_filter_on_color_img, flower_img, [1, 2], True)
        self.assertRaises(ValueError, imkernel.apply_filter_on_color_img, flower_img, ['blure', 'Laplasian'], True)
        self.assertRaises(ValueError, imkernel.apply_filter_on_color_img, flower_img, [], True)
        self.assertRaises(TypeError, imkernel.apply_filter_on_color_img, flower_img, ('blur',), True)
        self.assertRaises(TypeError, imkernel.apply_filter_on_color_img, flower_img, tuple([1, 2, 3]), True)
        self.assertRaises(TypeError, imkernel.apply_filter_on_color_img, ['blur'], 'blur', True)
